...
ESX.RegisterServerCallback('esx_documents:deleteDocument', function(source, cb, id)
    ...
    MySQL.Async.execute('DELETE FROM user_documents WHERE id = @id AND owner = @owner',
    {
        ...
        if rowsChanged >= 1 then
            xPlayer.showNotification(TranslateCap('document_deleted'))
            cb(true)
        else
            xPlayer.showNotification(TranslateCap('document_delete_failed'))
            cb(false)
        end
        end)
end)
...
AddEventHandler('esx_documents:CopyToPlayer', function(targetID, aForm)
    ...
    local _source, xPlayer_source   = source, ESX.GetPlayerFromId(source)
            ...
            if id ~= nil then
                MySQL.Async.fetchAll('SELECT * FROM user_documents where id = @id', {['@id']=id}, function (result)
                    ...
                    if(result[1] ~= nil) then
                        ...
                        targetxPlayer.showNotification(TranslateCap('copy_from_player'))
                        xPlayer_source.showNotification(TranslateCap('from_copied_player'))
                    else
                        xPlayer_source.showNotification(TranslateCap('could_not_copy_form_player'))
                    end
                end)
            else
                xPlayer_source.showNotification(TranslateCap('could_not_copy_form_player'))
